...
import Data.ByteString.Lazy.Char8 as L (ByteString, hPutStrLn, readFile,pack)
...
import System.Process.ByteString.Lazy as SPBL
...
-- | sendGmailBis. Like sendGmail allowing attachments as process results
sendGmailBis
  :: L.Text     -- ^ username
  -> L.Text     -- ^ password
  -> Address    -- ^ from
  ...
  -> IO ()
sendGmailBis user pass from to cc bcc subject body attachments micros = do
  ...
  bracket handle hClose $ \ hdl -> do
    ...
    bracket context cClose $ \ ctx -> do
      ...
      where username  = encode $ encodeUtf8 user
            ...
            mail      = renderMailBis from to cc bcc subject body attachments
-- |
...
sendGmail
  ...
  -> Int        -- ^ timeout (in microseconds)
  -> IO ()
sendGmail user pass from to cc bcc subject body attachments micros = sendGmailBis user pass from to cc bcc subject body (map AFile attachments) micros 
...
-- | Attachment as a file or as a result of a synchronous process
data Attachment = AFile 
                FilePath -- ^ filename
                | AProcess 
                FilePath -- ^ process command
                [String] -- ^ arguments
                (Maybe L.ByteString) -- ^ input to command
                FilePath -- ^ fake filename with extension for mime and attachment name
                deriving Show
renderAttachment (AFile path) = do
    content <- readFile path
    let mime = getMime $ takeExtension path
        file = Just . S.pack $ takeFileName path
    return $! [Part mime Base64 file [] content]

renderAttachment (AProcess command args input path) = do
    (_,content,za) <- SPBL.readProcessWithExitCode command args $ maybe "" id input
    print ("ciao",content,za)
    let mime = getMime $ takeExtension path
        file = Just . S.pack $ takeFileName path
    return $! [Part mime Base64 file [] content]
...
renderMail from to cc bcc subject body attach = 
        renderMailBis from to cc bcc subject body $ map AFile attach
...
renderMailBis
  ...
  -> [Attachment] -- ^ attachments
  -> IO L.ByteString
renderMailBis from to cc bcc subject body attach = do
  parts <- forM attach renderAttachment
